		...
		module Ratings
			def like
				if params[:place]
					...
					if @place != nil
						...
						if relationship_exists && 60.days.since(relationship_exists.updated_at) <= Time.now
			...
			def good_experience
				if params[:place]
					@place = Place.find_by_uuid(params[:place])
					if @place != nil
						relationship_exists = Relationship.find(:first,	:conditions => ["source_uuid = ? AND target_uuid = ? AND relationship IN ('good_experience','bad_experience')", @fbuser.uuid, @place.uuid],
																		:order => "updated_at DESC")

						if relationship_exists.nil? || (relationship_exists && 7.days.since(relationship_exists.updated_at) <= Time.now)
							new_rel = Relationship.new do |r|
								r.source_uuid = @fbuser.uuid
								r.target_uuid = @place.uuid
								r.relationship = "good_experience"
							end
							new_rel.save!
