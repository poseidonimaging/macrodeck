module MacroDeck
	module Places
		module Minifeed
			...
			def generate_fbml_for(minifeed)
				fbml = ActionView::Base.new(Rails::Configuration.new.view_path).render_to_string(:partial => 'facebook/home/minifeed', :locals => {:minifeed => minifeed})
			end

			# Generate a minifeed array for rendering
			def generate_minifeed_for(user)
				# Build the minifeed
				minifeed = []
			...
			def minifeed_patron_friends(user)
				minifeed_items = []
			...
			def minifeed_nudges_outgoing(user)
				minifeed_items = []
			...
			# Nudges sent to you
			def minifeed_nudges_incoming(user)
				minifeed_items = []
