module MacroDeck
	module Places
		module Recommendations
			def generate_fbml_for(recommendations)
				fbml = ActionView::Base.new(Rails::Configuration.new.view_path).render_to_string(:partial => 'facebook/home/recommendations', :locals => {:recommendations => recommendations})
			end
			...
			def recommendations_places(user)
			...
			def recommendations_popular_places(user)
				recommended_items = Array.new
			...
			def find_local_friend_relationships(user)
				if user.home_city && user.secondary_city
