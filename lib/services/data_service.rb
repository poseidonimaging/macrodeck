...
class DataService < BaseService
	...
	def self.getDataGroupOwner(gropuingID)
		group = DataGroup.find(:first, :conditions => ["grouping = ?", groupingID])
		return group.owner
	end
	...
	def self.findDataGrouping(type, searchBy, query)
		case searchBy
			when :parent, "parent"
				return DataGroup.findGroupingsByParent(type, query)
			when :creator, "creator"
				return DataGroup.findGroupingsByCreator(type, query)
			when :owner, "owner"
				return DataGroup.findGroupingsByOwner(type, query)
			when :type, "type"
				return DataGroup.findGroupings(type)
			else
				return nil
		end
	end
