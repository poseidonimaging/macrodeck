...
set :deploy_to, "/home/railsapps/#{application}" # defaults to "/u/apps/#{application}"
set :user, "railsapps"            # defaults to the currently logged in user
...
desc "Backup MySQL to /tmp/dump.sql"
task :backup, :roles => :db, :only => { :primary => true } do
  # the on_rollback handler is only executed if this task is executed within
  # a transaction (see below), AND it or a subsequent task fails.
  on_rollback { delete "/tmp/dump.sql" }

  run "mysqldump -u macrodeck -p macrodeck > /tmp/dump.sql" do |ch, stream, out|
    ch.send_data "iRKm$9wu\n" if out =~ /^Enter password:/
  end
end
