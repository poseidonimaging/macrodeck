class Facebook::HomeController < ApplicationController
	...
	def index
		@minifeed = []
		@minifeed += minifeed_nudges_incoming unless minifeed_nudges_incoming.nil?
	...
	private
		def minifeed_nudges_incoming
			...
			nudges = Relationship.find(:all,
									   :conditions => ["(target_uuid = ? AND subject IS NOT NULL AND relationship = 'nudged') OR (source_uuid = ? AND subject IS NULL AND relationship = 'nudged')",
